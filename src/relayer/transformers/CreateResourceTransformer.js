...
export default class CreateResourceTransformer extends PrimaryResourceTransformer {
  transformResponse(endpoint, response) {
    return response.then(
      (resolvedResponse) => {
        var resource = this.primaryResourceMapperFactory(resolvedResponse.data, this.ResourceClass).map();
        resource.templatedUrl.etag = resolvedResponse.etag;
        return resource;
      }
    ).catch(
      (resolvedError) => {
        throw this.primaryResourceMapperFactory(resolvedError.data, this.ResourceClass.errorClass).map();
      }
    );
  }
}
